...
class SongsList extends Component<Props, State> {
  ...
  componentDidMount() {
    ...
    let updateFunc = cachedList.length > 0
      ? () => {}
      : (currentList) => {
  ...
  render() {
    ...
    const validSongs = songs.filter(song => {
      ...
      if (filterLower.length > 0) {
        return title.toLowerCase().includes(filterLower) || artist.toLowerCase().includes(filterLower) || album.toLowerCase().includes(filterLower);
      }
      return true;
    });
    ...
    return (<div className='songs_container'>
      <ResizableTable onClicked={(row) => {playParams(row.playParams);}} initialColumnSizes={[45, 5, 20, 30]} columns={['name', 'time', 'artist', 'album']} columnNames={['Name', 'Time', 'Artist', 'Album']} rows={rows}/>
    </div>);
